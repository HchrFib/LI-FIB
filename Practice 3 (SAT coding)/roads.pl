...
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.
    ...
    numMaxStation:- maxStations(K), findall(install(C), city(C), Lits), atMost(K,Lits), fail.
    numMaxStation.

    installGasStation:- city(C1), city(C2), road(C1,C2), writeOneClause([install(C1) , install(C2)]), fail.
    installGasStation.

%%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

% displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- write(M).
