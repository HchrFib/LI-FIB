...
differentsTopic:- expertise(S1,E1), expertise(S2,E1), S1 \= S2,  writeOneClause([-chosen(S1), -chosen(S2)]), fail.
differentsTopic.

oneTeamWithExactlyKstudents:-  numMembersTeam(K), findall(chosen(S), student(S) ,  Lits), exactly(K,Lits), fail. 
oneTeamWithExactlyKstudents.

friends:-  student(S1), student(S2), not(friends(S1, S2)), S1 \= S2, writeOneClause([-chosen(S1), -chosen(S2)]), fail.  
friends.
