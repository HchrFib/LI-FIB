...
satVariable( doesGH(G,H) ):- gangster(G), hour(H).  %  means:  "gangster G work at hour H"     (MANDATORY)
...
writeClauses(MaxConsecutiveHours) :-
    eachGangsterWorkExactly(MaxConsecutiveHours),
    noMoreTwoConsecutivesHours(MaxConsecutiveHours),
    blockedGangsters(MaxConsecutiveHours),
    relationVariables(MaxConsecutiveHours),
    eachHourNeededKGangsters(MaxConsecutiveHours),
    eachGangsterAtMostWorkKconsecutivesHours(MaxConsecutiveHours),
    true,!.
writeClauses(_) :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachGangsterWorkExactly(_):-  gangster(G), hour(H), findall( does(G,T,H), task(T), Lits ), atMost(1, Lits), fail. 
eachGangsterWorkExactly(_).


noMoreTwoConsecutivesHours(_):-    gangster(G), task(T1), task(T2), T1 \= T2, 
                                hour(H1), H2 is H1 + 1, hour(H2),
                                writeOneClause([-does(G,T1,H1), -does(G,T2,H2)]), fail.
noMoreTwoConsecutivesHours(_).


blockedGangsters(_):- notAvailable(G,L), member(H,L), hour(H), writeOneClause([-doesGH(G,H)]),fail.
blockedGangsters(_).


relationVariables(_):- gangster(G), task(T), hour(H), writeOneClause([-does(G,T,H), doesGH(G,H)]), fail.
relationVariables(_).


eachHourNeededKGangsters(_):- needed(T,H,N), findall(does(G,T,H), gangster(G), Lits), exactly(N, Lits), fail. 
eachHourNeededKGangsters(_).
eachGangsterAtMostWorkKconsecutivesHours(K):- 
                                                gangster(G),
                                                AuxH is 72 - K,
                                                between(1,AuxH,H1),
                                                \+blocked(G,H1),
                                                H2 is H1 + K, hour(H2),
                                                findall(-doesGH(G,H), between(H1,H2,H), Lits),
                                                writeOneClause(Lits), 
                                                fail.
                                                
eachGangsterAtMostWorkKconsecutivesHours(_).
...
%% "costOfThisSolution(M,Cost) es cierto, si Cost es el maximo de horas consecutivas de todos los gangsters G"
costOfThisSolution(M,Cost):-    findall(Max, maxEachGangster(M, Max), Lits), sort(Lits,L1),  append(_,[Cost],L1),!.

%% "maxEachGangster(Model, Max) es cierto, si Max es el maximo de horas conscutivas en la que ha trabajado el gangster G"
maxEachGangster(Model, Max):-      list(Model, L), findall(N, subsec(L, N), Lits), sort(Lits,L1),  append(_,[Max],L1).

%% "list(L,N) es cierto, si Lits contiene todas las horas en las que ha trabajado el gangster G"
list(Model, Lits):-             gangster(G), task(T), findall(H, member(does(G,T,H),Model), Lits).

%% "subsec(L,N) es cierto, si N es el numero de horas consecutivas que el gangster G ha trabajado"
subsec(L,N):- append(_,END,L), append(SS,_,END), consecutivos(SS, C), length(C, N).

consecutivos([X],[X]).
consecutivos([X,Y|L], [X|SP]):- Y is X + 1, consecutivos([Y|L],SP).
