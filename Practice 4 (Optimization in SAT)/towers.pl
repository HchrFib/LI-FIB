...
satVariable( towerVill(V)):- village(V).
...
writeClauses(MaxNumTowers):-
        ...
        %relationVariables
        expressOrTowerVillage,
        posWatchesVillage,
        significantVillage,
        onlyTowerInVillages,
        minNumTowers(MaxNumTowers),
...
expressOrTowerVillage:-  village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), expressOr(towerVill(V), Lits), atMost(1, Lits),fail.
expressOrTowerVillage.

relationVariables:- posVillage(V,I,J), writeOneClause([-towerPos(I,J), towerVill(V)]), fail.
relationVariables.

posWatchesVillage:- village(V), findall(towerPos(I,J), posWatchesVillage(V,I,J), Lits), atLeast(1, Lits), fail.
posWatchesVillage.

significantVillage:- significantVillage(V), findall(towerPos(I,J), posVillage(V, I,J), Lits), exactly(1, Lits), fail. 
significantVillage.

onlyTowerInVillages:- position(I,J), not(posVillage(_,I,J)), writeOneClause([ -towerPos(I,J) ]), fail.
onlyTowerInVillages.

minNumTowers(MaxNumTowers):-  findall(towerVill(V), village(V) ,Lits ), atMost(MaxNumTowers, Lits), fail.
minNumTowers(_).
...
costOfThisSolution(M,Cost):- findall(towerPos(I,J) , member(towerPos(I,J) , M),Lits), length(Lits, Cost),!.
